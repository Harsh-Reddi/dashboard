        ...
        <div className='px-2 lg:px-7 pt-5'>
            <div className='w-full p-4 bg-[#527e2f] text-[#d0d2d6] rounded-md'>
                <div className='flex justify-between items-center pb-4'>
                    <h1 className='text-[#d0d2d6] font-semibold'>Edit Product</h1>
                    ...
                    <form onSubmit={update}>
                        ...
                        <div className='flex flex-col mb-3 md:flex-row gap-4 w-full text-[#d0d2d6]'>
                                    ...
                                    <div className='flex justify-start items-start flex-col pt-6 h-[220px] overflow-x-scrool'>
                                        {
                                            allCategory.map((c, i) => <span className={`px-4 py-2 hover:bg-[#527e2f] hover:text-[#d0d2d6] hover:shadow-lg w-full cursor-pointer ${category === c.name && 'bg-[#527e2f]'}`} onClick={() => {
                                                ...
                                                console.log("Selected category:", c.name); // Debugging line
                                                setCateShow(false)
                                                setCategory(c.name)
                                                setSearchValue('')
                                                setAllCategory(categories)
                            ...
                            <div className='flex flex-col w-full gap-1'>
                                ...
                                <input className='px-4 py-2 focus:border-[#527e2f] outline-none bg-[#527e2f] border border-slate-700 rounded-md text-[#d0d2d6]' onChange={inputHandle} value={state.stock} type="text" name='stock' id='stock' placeholder='Stock' />
                            </div>
                        </div>
                        <div className='flex flex-col mb-3 md:flex-row gap-4 w-full text-[#d0d2d6]'>
                            <div className='flex flex-col w-full gap-1'>
                        ...
                        {
                            (imgShow && imgShow.length > 0) && imgShow.map((img, i) => (
                                <div key={i}>
                                    <label htmlFor={`image-${i}`}>
                                        <img src={img} alt={`${i}`} className='h-[180px] w-[280px]'/>
                                    </label>
                                    <input
